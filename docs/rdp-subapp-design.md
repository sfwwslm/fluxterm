...
- 后续如果依赖冲突被稳定消除，RDP 运行时优先回收至 `crates/rdp_core`，而不是重新散落回 `src-tauri`
...
- 将真实运行时回收到 workspace 内的 `crates/rdp_core`
...
- 已新增 `crates/rdp_core` 作为 workspace 内独立 RDP 核心 crate
- 已将 `src-tauri/src/rdp.rs` 改为对 `rdp_core` 的编排与状态代理
...
- `crates/rdp_core`：在 workspace 路线下负责真实 `IronRDP` 会话、图像解码、输入下发与本地 bridge
...
- `crates/rdp_core/`：workspace 内独立 RDP 核心 crate
