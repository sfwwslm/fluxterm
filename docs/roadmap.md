...
目标：从文件管理器直接打开远程文件，并通过系统编辑器完成文本类文件编辑与回传，方便修改远程设备上的配置文件、脚本和文本文件。

当前方案：

- 不自研编辑器，统一复用系统默认程序或用户配置的默认编辑器
- 远端文件下载到应用受管缓存目录后打开
- 文本类文件进入“保存后确认回传”闭环，非文本文件只负责打开
- 回传前检查远端文件是否已变化，避免静默覆盖

剩余工作：

- 补充单独设计文档，固化缓存目录、轮询检测和冲突策略
